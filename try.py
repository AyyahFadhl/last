...
def Wrapper2(p, TargetPayload):
    def Custom_loss(y_true, y_pred=0.4):
        # Calculating Adverserial Loss
        ld = cross_entropy(y_true, y_pred)
        # Calculating loss in Capacity
        # Calculate Capacity using probability Map.
...
def GAN(input_shape=(256, 256, 1)
    ...
    discriminator.compile(loss=DiscriminatorLossFunction,optimize=discriminator_optimizer )

    #Generator
    generator_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001)
    generator=FCDenseNet(n_filters_first_conv=48
                   , Filter_size_first_conv=3
                   , n_path_layer=4
                   , growth_rate=12
                   , n_layers_per_block=5
    ...
    CoverImage=tf.keras.layers.Input(input_shape=(256,256,1))
    ...
    GAN.compile(loss=Wrapper(generator.layers[-1].output, payload), optimizer=generator_optimizer)
...
def Train(epoch=100, batch_size=1, save_interval=1):
    ...
    for epoch in range(epoch):
        ...
        GAN_Evaluation.QualitativeEvaluation(epoch, generator, TestX, epochTrainingTime)
        # 3. Quantitative Evaluation
        GAN_Evaluation.QuantitativeEvaluation()
...
def lossGraph(self, epoch, G_Loss, D_Loss):
    os.makedirs(os.path.join(self.MasterPath, r'TraininglossGraphs'), exist_ok=True)
    plt.plot(G_Loss, 'r-', label='G_Loss')
    plt.plot(D_Loss, 'k-', label='D_Loss')
    plt.xlabel('iteration')
    plt.ylabel('loss')
    plt.legend()
    plt.savefig(os.path.join(self.MasterPath, f'TraininglossGraphs\epoch {epoch + 1}.png'))
    plt.close()
    return
